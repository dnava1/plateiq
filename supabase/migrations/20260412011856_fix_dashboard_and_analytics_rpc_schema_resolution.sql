...
BEGIN
  SELECT jsonb_build_object(
    ...
    'recent_workouts', (
      ...
      JOIN public.exercises e ON e.id = w.primary_exercise_id
    ),
    ...
    'current_tms', (
      SELECT COALESCE(
        jsonb_agg(
          ...
          ORDER BY e.name, tm.effective_date DESC
        ),
        '[]'::jsonb
      )
...
GRANT EXECUTE ON FUNCTION public.estimate_one_rep_max(numeric, int) TO authenticated;
