...
historical_main_lift_amrap_sets AS (
	SELECT
		ws.exercise_id,
		ws.reps_actual,
		ws.weight_lbs,
...
stall_detection_rows AS (
	...
	FROM historical_main_lift_amrap_sets
...
coverage_counts AS (
	SELECT
		...
		(SELECT COUNT(*) FROM main_lift_amrap_sets) AS main_lift_amrap_count,
		...
		SELECT jsonb_build_object(
			'metrics', jsonb_build_object(
				...
				'e1rm_trend', jsonb_build_object(
					...
					'status', CASE
						WHEN cc.main_lift_amrap_count > 1 THEN 'ready'
						WHEN cc.main_lift_amrap_count = 1 THEN 'limited'
						WHEN cc.main_lift_set_count > 0 THEN 'limited'
						WHEN cc.bodyweight_set_count > 0 THEN 'not_applicable'
						ELSE 'not_applicable'
					END,
					'signal_count', cc.main_lift_amrap_count,
					'reason_codes', CASE
						WHEN cc.main_lift_amrap_count = 1 THEN jsonb_build_array('limited_history')
						WHEN cc.main_lift_amrap_count = 0 AND cc.main_lift_set_count > 0 THEN jsonb_build_array('no_amrap_sets')
						WHEN cc.main_lift_amrap_count = 0 AND cc.bodyweight_set_count > 0 THEN jsonb_build_array('bodyweight_only_scope')
						WHEN cc.main_lift_amrap_count = 0 THEN jsonb_build_array('no_main_lift_sets')
						ELSE '[]'::jsonb
					END
				),
