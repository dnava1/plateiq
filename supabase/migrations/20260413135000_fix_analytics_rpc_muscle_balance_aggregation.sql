  ...
  SELECT jsonb_build_object(
    ...
    'muscle_balance', (
      SELECT COALESCE(
        jsonb_agg(
          jsonb_build_object(
            'movement_pattern', balance.movement_pattern,
            'total_volume',     balance.total_volume,
            'volume_pct',       balance.volume_pct
          )
        ),
        '[]'::jsonb
      )
      FROM (
        SELECT
          movement_totals.movement_pattern,
          movement_totals.total_volume,
          ROUND(
            (movement_totals.total_volume * 100.0 /
             NULLIF(SUM(movement_totals.total_volume) OVER (), 0)
            )::numeric,
            1
          ) AS volume_pct
        FROM (
