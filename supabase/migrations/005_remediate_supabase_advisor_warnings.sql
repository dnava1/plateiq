...
-- Remediate fixable Supabase advisor findings.
-- Leaked password protection is an Auth/dashboard setting and is intentionally not handled in SQL.
...
ALTER POLICY "read exercises"
  ON public.exercises
  USING (created_by_user_id IS NULL OR created_by_user_id = (select auth.uid()));

ALTER POLICY "insert own exercises"
  ON public.exercises
  WITH CHECK (created_by_user_id = (select auth.uid()));

ALTER POLICY "update own exercises"
  ON public.exercises
  USING (created_by_user_id = (select auth.uid()))
  WITH CHECK (created_by_user_id = (select auth.uid()));

ALTER POLICY "delete own exercises"
  ON public.exercises
  USING (created_by_user_id = (select auth.uid()));

ALTER POLICY "all own data"
  ON public.training_maxes
...
CREATE INDEX IF NOT EXISTS idx_exercises_created_by_user_id
  ON public.exercises (created_by_user_id);

CREATE INDEX IF NOT EXISTS idx_training_maxes_exercise_id
  ON public.training_maxes (exercise_id);

CREATE INDEX IF NOT EXISTS idx_workout_sets_exercise_id
  ON public.workout_sets (exercise_id);

CREATE INDEX IF NOT EXISTS idx_workouts_primary_exercise_id
  ON public.workouts (primary_exercise_id);
